...
/** @deprecated Prefer getProductById / MOCK_PRODUCTS — kept for older imports. */
export const MOCK_PRODUCT = MOCK_PRODUCTS[0];
...
export function initialStockMap(): Record<string, number> {
  return Object.fromEntries(MOCK_PRODUCTS.map((p) => [p.id, p.stock]));
}
